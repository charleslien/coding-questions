...
def check_network_error(t: str) -> tuple:
    """
    Determine if the received message could be the result of a network error
    where two identical messages were merged.
    
    Args:
        t: Non-empty string consisting of lowercase Latin letters (length ≤ 100)
        
    Returns:
        Tuple (result, original_message), where:
        - result: "YES" if the message could be the result of an error, "NO" otherwise
        - original_message: Possible original message if result is "YES", empty string otherwise
    """
    pass
